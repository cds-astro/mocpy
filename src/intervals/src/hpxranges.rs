      ...
      T: Idx + CheckedAdd,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        while !self.ranges.is_empty() {
            ...
            // new_from_sorted?
            let mut buffer_ranges = HpxRanges::<T>::new_from(self.buffer.clone());
      ...
      T: Idx + CheckedAdd,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        while self.id < self.ranges.0.0.len() {
        ...
        if next_depth_pix.is_some() {
            ...
            while !self.ranges.is_empty() {
                for range in self.ranges.iter() {
                    ...
                    if c2 > c1 {
                        let mut range_to_remove = HpxRanges::<T>::new_unchecked(vec![c1..c2]);
