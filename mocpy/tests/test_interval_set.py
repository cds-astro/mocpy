...
def test_interval_set_union(isets):
    assert isets["a"].union(isets["b"]) == IntervalSet(
        np.array([[9, 126]], dtype=np.uint64)
    )
    assert isets["a"].union(IntervalSet()) == IntervalSet(
        np.array([[27, 126]], dtype=np.uint64)
    )
    assert IntervalSet().union(isets["a"]) == IntervalSet(
        np.array([[27, 126]], dtype=np.uint64)
    )


def test_interval_set_intersection(isets):
    assert isets["a"].intersection(isets["b"]) == IntervalSet(
...
def test_interval_set_difference(isets):
    assert isets["a"].difference(isets["b"]) == IntervalSet(
        np.array([[61, 68], [105, 126]], dtype=np.uint64)
    )
    assert isets["b"].difference(isets["a"]) == IntervalSet(
